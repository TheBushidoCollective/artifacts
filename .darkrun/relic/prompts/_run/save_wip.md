...
> **Run** `relic` · **Phase** `save_wip`


# Save Work in Progress — `main`

You have **uncommitted work** in the project tree. The manager will not advance the run with loose changes on `main` — commit them first, then retick. This is purely mechanical: no human intervention needed.
...
3. Commit everything listed above on `main` (`git add … && git commit …`).
