...
5. **Close the loop**: record what you changed and why on `fb-09`, and resolve it.

## Done when

`fb-09` is resolved with evidence, the artifact is corrected, and nothing else regressed. Then call `darkrun_tick` — if more feedback is open, the manager routes the next item; otherwise the run resumes.