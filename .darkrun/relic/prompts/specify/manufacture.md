...
### `spec-relic-format` — Specify the URL, the relic ID, and the ciphertext container

- **inputs:** `frame.md`


- **outputs:** `docs/spec/format.md`, `docs/spec/format.sources.txt`


- **quality gates:** artifact-exists — `test -f docs/spec/format.md` · substance-floor — `test "$(wc -w < docs/spec/format.md)" -ge 1600` · sources-manifest-populated — `bash -c 'set -eu; n=$(grep -c . docs/spec/format.sources.txt); test "$n" -ge 5'` · every-cited-url-resolves — `bash -c 'set -eu; while IFS= read -r u || [ -n "$u" ]; do [ -n "$u" ] || continue; curl -sfL --max-time 25 --retry 2 -A "Mozilla/5.0 (relic-link-check)" -o /dev/null "$u"; done < docs/spec/format.sources.txt'`
...
- `spec-relic-format` → `/Users/jwaldrip/dev/src/github.com/thebushidocollective/artifacts/.darkrun/worktrees/relic/units/specify/spec-relic-format` (branch `darkrun/relic/units/specify/spec-relic-format`)
