...
> **Run** `relic` · **Station** `specify` · **Phase** `review`
...
Persist it: call `darkrun_brief_record` with `slug: relic`, `station: specify`, `phase: pre`, and the brief as `body`. This is the pre-execution brief the operator's review gate surfaces — a durable artifact, not inline prose.
