...
approvals:
  ...
  simplicity:
    at: 2026-08-02T21:08:33.116191+00:00
quality_gates:
...
- name: sources-manifest-populated
  status: pass
  at: 2026-07-30T12:24:20.237782+00:00
  attempts: 1
  detail: |-
    13 non-empty lines against a floor of 6, one URL per line, trailing newline. Run by the manager at `a07dafd`.

    Unchanged across all three beats. Two of the resolve pass's fixes were specifically written to need **no new citation**: the 255-octet cap replaced a false filesystem claim with pure arithmetic (a 255-octet UTF-8 string is never more than 255 characters and never more than 255 UTF-16 code units), and the `rs - 17` block's two new benefits rest on evidence the beat generated by running the reference implementation rather than on a source. Two-way orphan check exact at 13 and 13.
- name: every-cited-url-resolves
  status: pass
  at: 2026-07-30T12:24:29.345643+00:00
  attempts: 1
  detail: |-
    All 13 URLs fetched, exit 0, no DEAD lines. Run by the manager at `a07dafd`. Orphan check clean both directions.

    **This gate is the weakest check in the run and this unit is the clearest demonstration of why.** It passed identically at the make pass, when the quotation audit was in fact 42 audited with 2 deviations while reporting 35 of 35 verbatim. Every URL resolved in both states.

    **The audit is now genuinely clean and, more usefully, mechanically checkable.** Final state 50 audited, 50 verbatim, zero deviations, covering 39 double-quoted runs and 11 backticked source strings. The resolve pass did something better than fix the count: it **converted the four remaining scare-quotes to non-quotation form, so every double-quoted run in the document is now a source quotation.** Criterion 9 no longer requires judgment about which quotes count.

    Both prior deviations are worth recording because of what they show about the failure mode. One was a case-altered initial letter against `format.md`, **the identical class the make pass had caught and corrected three sections earlier in the same document**, which is evidence that catching an instance does not inoculate against the pattern. The other elided a bracketed RFC cross-reference and moved a period inside the quotation marks.

    **A fourth false-negative mode surfaced and is recorded for the next checker:** an apparent third deviation was the checker's own extraction artifact. The GFM punctuation quote fails if `<code>` tags are stripped to bare text and is verbatim once code spans are preserved as backticks. That joins page furniture, hyphenation at line breaks, and typographic punctuation as ways this check manufactures a false accusation.
---
