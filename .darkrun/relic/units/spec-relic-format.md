...
2. `test "$(wc -w < docs/spec/format.md)" -ge 2000` exits 0. **Calibration, stated honestly:** this unit carries roughly 27 mandated items, and the observed rate in this run is 60 to 85 words per item, so a compliant document lands somewhere between about 1600 and 2300 words. **2000 sits inside that band rather than below it**, so if you find yourself near the floor, check whether you have padded before assuming you are short. It is a stub guard with a narrower margin than the sibling units', not a target.
