...
**A second honesty constraint, which JSX rendering created.** The usercontent frame renders untrusted HTML, and now JSX as well, and it may reach the network from inside the recipient's browser. That is a new capability for untrusted content, not a wider sandbox for an old one: whoever authored a relic can learn the recipient's IP, user agent, and open time by pointing that reach at a host they control, and no boundary changes that, because the exposure is the recipient's own request rather than the relic's bytes. What the origin boundary still prevents is the key. Rendered content executes on an opaque origin that can never read the fragment on the viewing origin, so what an author buys is information about the recipient, never the key or anything held on the viewing origin. The published disclosure and the viewer's own copy both state the first half plainly, because an unstated capability would turn `/policy` into an overclaim, and this repo treats an overclaim as a defect.
