...
6. **The network capability of rendered content.** Content that renders on the usercontent origin, HTML and JSX, executes and may reach the network from inside the recipient's browser, so whoever authored the relic can learn the recipient's IP, user agent, and open time by pointing that reach at a host they control. Stated as a capability of the content rather than a feature of Relic: the origin boundary still keeps such content away from the decryption key, which never leaves the link and the recipient's browser, and nothing keeps it away from the recipient's own connection. The viewer carries the same statement on the relic page before the content renders (`spec-viewer`).
