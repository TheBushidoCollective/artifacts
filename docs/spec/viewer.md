...
**The frame imports React and ReactDOM from a CDN itself, because it cannot import them from Relic.** The render frame is an opaque origin, so its module fetches are CORS requests carrying `Origin: null`, and Relic's own origins do not answer those. A permissive CDN does, because it sends `Access-Control-Allow-Origin: *`, which is why the frame's dependency chain runs through one and why the frame's own bootstrap is inlined by the build rather than fetched: same-origin assets are the one thing this origin structurally cannot reach.

**The network reach is a disclosed capability, not a leak being papered over.** A JSX relic, and an HTML relic on the same frame, can fetch from any permissive host, so whoever authored the relic can learn the recipient's IP, user agent, and open time by pointing that reach at a host they control. The boundary still does its job: the frame is opaque, holds no credentials, cannot read the fragment on the viewing origin, and cannot reach anything of Relic's that trusts the request. What it cannot do is protect the recipient's own connection, and §6.3's copy says so on the page before the content renders, because a recipient who discovers an unstated capability stops trusting the stated ones.
...
- **Layer one, the shim.** A static file on the per-relic usercontent origin, served on a real origin with no `sandbox` on its own response, so the parent can name it with an exact `targetOrigin` and it can name the parent with one. It's first-party code Relic authors. It never touches ciphertext, never touches the key, and its own code makes no requests. The documents it renders are the networked half now (§3.5), and that reach belongs to the content, not to the shim: the origin still serves one static file, and the bytes that leave the recipient's browser leave as the relic's own fetches. Its own response carries `default-src 'none'` with its single inline script admitted by hash, `frame-ancestors` limited to the viewing origin, and `X-Content-Type-Options: nosniff`.
- **Layer two, the render frame.** The shim builds the untrusted document and renders it in a child iframe carrying the `sandbox` attribute. That's where the opaque origin belongs and where it costs nothing, because nothing needs to address the render frame by origin. **Never both `allow-scripts` and `allow-same-origin`**, which is locked in the preconditions. Outbound requests are not blocked, and the reversal that brought JSX rendering made that deliberate rather than dormant: the render document may load modules and resources over HTTPS, because a JSX component that could not import React could not render at all. The reach is bounded by what the opaque origin is: no credentials, no storage, no same-origin relationship with anything, and no path to the viewing origin's fragment. It is disclosed to the recipient (§6.3) rather than prevented, and nobody reads this layer as confinement of the recipient's network.
...
**Present the sandbox as deliberate.** An HTML relic can't navigate the top-level window or open popups, and it can't reach the decryption key. It can load external resources, and the taskbar says so before the content renders. Left unexplained, the recipient concludes the relic is corrupt and the publisher concludes the product is broken. The taskbar says the content is sandboxed and says what that still blocks, and the list it gives is the true one: top-level navigation, popups, and everything same-origin. The network is not on it.
...
**The honesty constraint applies hardest here.** "Nobody can read this but you" is an overclaim on the one surface where it does the most damage. The decryption JavaScript is served by the same operator the claim is made against, so it's a claim about operator intent rather than a property the recipient can verify. The copy says what's true: the key stays in the link and this browser, and the recipient's browser never sends it to Relic's servers. For content that renders on the usercontent origin, one more sentence is true and gets said on the same page: the content itself can reach the network, whoever authored it can learn the recipient's IP, user agent, and open time that way, and the sandbox contains it away from the key, not away from the recipient's connection.
