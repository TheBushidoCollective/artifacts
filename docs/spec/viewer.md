...
**The honesty constraint applies hardest here.** "Nobody can read this but you" is an overclaim on the one surface where it does the most damage. The decryption JavaScript is served by the same operator the claim is made against, so it's a claim about operator intent rather than a property a recipient can verify. The copy says what's true: the key stays in the link and this browser, and the recipient's browser never sends it to Relic's servers. For content that renders on the usercontent origin, one more sentence is true and gets said on the same page: the content cannot reach the network at all, because the frame is served a policy that permits no remote source, so whoever authored it has no channel to the recipient through the rendered page, and a page whose external assets are missing is seeing the block rather than a break.
