...
6. **Success with no history.** When `current_version == 1`, the taskbar carries no comparison control and the page says nothing about versions. Absence is deliberate. A recipient with nothing to compare should not have to parse a disabled control or an explanation for a feature that does not apply.
7. **History available.** When `current_version > 1`, the current plaintext is at or below the comparison ceiling, and its renderer is diffable, the taskbar adds **Compare versions**. Opening it defaults to `current_version - 1` and offers every retained earlier version in one selector. The heading and history statement always name numbers, never only relative words: **"Comparing version 2 with version 5"** and **"Version 5 is current. Version 2 is retained history and may contain content removed from the current artifact."** Changing the selector updates both numbers before the fetch starts. While the second mint, fetch, and decrypt run, the result area says **"Loading version 2 for comparison."** Fetching the comparison consumes one additional mint, exactly like any other signed download.
8. **Markdown comparison.** Markdown compares source lines, not rendered prose. Added and removed lines carry distinct backgrounds, and a one-line replacement emphasizes changed words inside the line. The mode label is **"Markdown source comparison"** followed by **"N added lines, N removed lines."** The plaintext and diff result stay in the recipient's browser.
9. **Code comparison.** Code uses the same line and intra-line algorithm under the mode label **"Code comparison."** It does not run either version and it does not trust a language hint to create elements or attributes outside the viewer's fixed diff vocabulary.
10. **HTML and JSX comparison.** Both compare source text on the service origin under the mode label **"Source text comparison."** The viewer says **"HTML and JSX are compared as source text. Neither historical source nor diff markup is sent to the rendered-content frame."** Rendered-DOM diffing is rejected here. It would execute two versions of author code, compare output that can depend on time and browser state, and require a second rendering protocol for a result that is already exact at the source layer. Source text is stable, works under the frame's network-denied policy, and leaves the existing one-render-per-frame rule untouched. The diff library is bundled into the service-origin viewer bundle and is absent from the usercontent frame bundle.
11. **Image comparison.** A changed raster image gets a swipe control that reveals the current version over the selected historical version. Both labels carry version numbers, the slider has the accessible name **"Reveal version 5 over version 2,"** and the summary states both dimensions and their signed delta: **"Version dimensions: 640 x 480 px to 800 x 450 px. Width +160 px, height -30 px."** This is visual comparison only. The viewer does not claim per-pixel or structural analysis.
...
13. **History present but comparison unavailable.** Download-only renderers do not get a broken control. The current page remains rendered and says **"Earlier versions exist, but this file is download-only. Relik cannot compare media, archives, or binary files in the browser."** A current plaintext above the ceiling also keeps rendering normally and says **"Earlier versions exist, but this version is larger than the 4 MiB comparison limit. The current version is still open normally."** A historical version above the ceiling is refused after its mint and before its ciphertext fetch, naming that version and confirming the current version remains open. Other historical fetch or decrypt failures stay inside the comparison result area and never replace the current relic's valid state.

**The comparison ceiling is 4 MiB per version, separate from the 100 MiB one-version render ceiling and the 100 MiB plaintext publication cap.** Two plaintext byte arrays cost 8 MiB. Decoding both into worst-case UTF-16 strings adds up to 16 MiB. The comparison fetch can hold up to another 8 MiB of ciphertext buffers. Reserving 32 MiB for the diff graph, line metadata, and DOM text gives a 64 MiB comparison working set before the browser's own page overhead. That margin is intentional for mobile Safari. A comparison that crashes the tab is worse than a named refusal.

**Version history does not change the trust boundary.** The viewer mints each selected ciphertext through the existing service path, decrypts both versions with the one fragment key, and computes the result client-side. No plaintext and no diff result reaches the service. The usercontent frame receives neither historical source nor diff output, and no comparison message type is added to its `postMessage` protocol.
